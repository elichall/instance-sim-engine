...
*/
// --- Constructor ---
GraphicsEngine::GraphicsEngine(int instanceShape, int maxInstanceCapacity,
                               std::array<int, 2> windowSize,
                               bool usingWireFrame,
                               std::array<int, 2> resolution,
                               bool usingLineFlag, int maxLinePoints,
                               bool dynamicColorFlag)
    : shape(instanceShape, resolution), maxCapacity(maxInstanceCapacity),
      winSize(windowSize), isFrame(usingWireFrame), isLine(usingLineFlag),
      maxLineSize(maxLinePoints), isDynamicColor(dynamicColorFlag),
      camera(windowSize) {
  lastX = windowSize[0] / 2.0;
  lastY = windowSize[1] / 2.0;
  isDragging = false;
}
...
bool GraphicsEngine::init() {
  ...
  window =
      glfwCreateWindow(winSize[0], winSize[1], "Instance Engine", NULL, NULL);
  if (window == NULL) {
  ...
  // area of the window we want open gl to render in (can integrate other things
  // into one window)
...
void GraphicsEngine::renderFrame(const RenderPayload &payload) {
  ...
  if (instObjectNum > maxCapacity) {
    ...
    glBufferData(GL_ARRAY_BUFFER, maxCapacity * sizeof(glm::mat4), nullptr,
                 GL_DYNAMIC_DRAW);

    glBindBuffer(GL_ARRAY_BUFFER, colorVBO);
    glBufferData(GL_ARRAY_BUFFER, maxCapacity * sizeof(glm::vec3), nullptr,
                 GL_DYNAMIC_DRAW);
  }
  ...
  if (isDynamicColor && payload.colors != nullptr) {
    glBindBuffer(GL_ARRAY_BUFFER, colorVBO);
    glBufferSubData(GL_ARRAY_BUFFER, 0, instObjectNum * sizeof(glm::vec3),
                    payload.colors->data());
  }
  // static colors upload once on the very first frame
  else if (!isDynamicColor && payload.colors != nullptr &&
           !staticColorsAssigned) {
    glBindBuffer(GL_ARRAY_BUFFER, colorVBO);
    glBufferSubData(GL_ARRAY_BUFFER, 0, instObjectNum * sizeof(glm::vec3),
                    payload.colors->data());
  ...
  glPolygonMode(GL_FRONT_AND_BACK,
                GL_FILL); // toggle solid fill (use instance colors)
  ...
  if (isFrame) {
    ...
    glDrawElementsInstanced(GL_TRIANGLES, numCubeIndices, GL_UNSIGNED_INT, 0,
                            instObjectNum);
  ...
  if (isLine && payload.lines != nullptr &&
      !payload.lines->empty()) { // double protection
    glUseProgram(lineShaderProgram);

    // pass camera matrices to the instance shader program
    glUniformMatrix4fv(lineViewLoc, 1, GL_FALSE, glm::value_ptr(viewMat));
    glUniformMatrix4fv(lineProjLoc, 1, GL_FALSE, glm::value_ptr(projMat));
    ...
    for (const auto &line : *payload.lines) {
      ...
      if (numPoints > maxLineSize) {
        ...
        // reallocate buffer
        glBufferData(GL_ARRAY_BUFFER, maxLineSize * sizeof(glm::vec3), nullptr,
                     GL_DYNAMIC_DRAW);
      }

      glUniform3fv(lineColorLoc, 1, glm::value_ptr(line.color));

      glBufferSubData(GL_ARRAY_BUFFER, 0, numPoints * sizeof(glm::vec3),
                      line.points.data());
...
bool GraphicsEngine::compileShaders() {
  ...
  const char *instancedVertexShaderSource =
      "#version 330 core\n"
  ...
  // GPU can't understand source code so we need to compile it and pass it to
  // GPU
...
bool GraphicsEngine::glObjects() {
  ...
  glBindBuffer(GL_ARRAY_BUFFER,
               geometryVBO); // binds what we do to VBO to the native gl array
                             // buffer of the same type

  // Pass data to GPU
  glBufferData(GL_ARRAY_BUFFER, // type of object
               shape.shapeVertices.size() *
                   sizeof(GLfloat),        // size of data array in bytes
               shape.shapeVertices.data(), // the actual data
               GL_STATIC_DRAW);            // what we want to do with it GL
  // static(mod one and used many times),dynamic(modified a lot),stream(mod one
  // and used a few times))_(draw,read,copy)
  ...
  glVertexAttribPointer(0,             // pos of vertext attribute
                        ...
                        dataPerVertex *
                            sizeof(float), // sprite of verticies (amount of
                                           // data between each vertex)
  ...
  glBufferData(GL_ARRAY_BUFFER, numOfInstanceObjects * sizeof(glm::mat4),
  ...
  glBindBuffer(GL_ARRAY_BUFFER, colorVBO);
  glBufferData(GL_ARRAY_BUFFER, numOfInstanceObjects * sizeof(glm::vec3),
               nullptr, GL_DYNAMIC_DRAW);

  int dataPerColor = 3;    // RBG
  glVertexAttribPointer(5, // not the number of particles but instead the
                           // location specified in shader
                        dataPerColor, GL_FLOAT, GL_FALSE, sizeof(glm::vec3),
                        (void *)0);
  ...
  glBufferData(GL_ELEMENT_ARRAY_BUFFER,
               ...
               shape.shapeIndices.data(), GL_STATIC_DRAW);
  ...
  if (isLine) {
    ...
    glVertexAttribPointer(0, nDimSpace, GL_FLOAT, GL_FALSE, sizeof(glm::vec3),
                          (void *)0);
    glEnableVertexAttribArray(0);
...
bool GraphicsEngine::shouldClose() { return glfwWindowShouldClose(window); }
// --- Deconstructor ---
...
void GraphicsEngine::mouseButtonCallback(GLFWwindow *window, int button,
                                         int action, int mods) {
...
void GraphicsEngine::cursorPositionCallback(GLFWwindow *window, double xposIn,
                                            double yposIn) {
  GraphicsEngine *engine =
      static_cast<GraphicsEngine *>(glfwGetWindowUserPointer(window));
...
void GraphicsEngine::scrollCallback(GLFWwindow *window, double xoffsetIn,
                                    double yoffsetIn) {
  GraphicsEngine *engine =
      static_cast<GraphicsEngine *>(glfwGetWindowUserPointer(window));
